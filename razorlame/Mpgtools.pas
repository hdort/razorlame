...
Warning - 
This file has been modified from its orginal form by Jon Dee
( potsticker@jon-dee.org )

Changes: exposed support for Xing Tag, Mp3Type variable.
These changes will be introduced into the main release of this file:
you should go to 

http://www.dv.co.yu/mgscript/mpgtools.htm
...
type
  ...
  { Definition for structure of MPEG AUDIO DATA records. Do not use it
    ...
    DurationExact : double;  { Song duration, more precise...}
...
type
  TMp3Type     = (mpVBR, mpCBR, mpUnknown);
  ...
  TMPEGAudio = class (TObject)
    private
      ...
      function  FGetNumberOfFrames : Integer;
      function FGetXingFlags : Integer;
      function FGetVBRScale : Integer;
      function FGetXingBytes : Integer;

      function FGetDurationExact : double;
    ...
    public
     ...
     { contents of TAG header }
      ...
      property Mp3Type : TMp3Type read FMp3Type write FMP3Type;
      property NumberOfFrames : Integer read FGetNumberOfFrames;
      property XingFlags : Integer read FGetXingFlags;
      property VBRScale  : Integer read FGetVBRScale;
      property XingBytes : Integer read FGetXingBytes;
     ...
     { Value that should be returned if Title field in tag is empty.
      ...
      property DurationExact : double read FGetDurationExact;
...
function TMPEGAUDIO.FGetDurationExact : double;
begin
     if FData.DurationExact = 0 then result:= (8*(FileLength - FirstValidFrameHeaderPosition)) / (Bitrate * 1000)
     else result:=FData.DurationExact;
end;
            ...
            If (not IsValid) or (TempLongInt <= 0) then begin
              ...
              If not DecodeHeader (mp3hdr, FData) then begin
                ...
                then begin
                  ...
                  fdata.DurationExact := ((1152 / fdata.samplerate) * xingHeader.frames);
...
function TMPEGAUDIO.FGetNumberOfFrames : Integer;
begin
     result:=0;
     if Mp3Type = mpVBR then
       result := XingHeader.frames
     else if Mp3Type = mpCBR then
       result := (FileLength- FirstValidFrameHeaderPosition) div FrameLength;
end;

function TMPEGAUDIO.FGetXingFlags : Integer;
begin
    result:=0;
    if Mp3Type = mpVBR then result := XingHeader.flags;
end;

function TMPEGAUDIO.FGetVBRScale : Integer;
begin
    result:=0;
    if Mp3Type = mpVBR then result := XingHeader.vbrscale;
end;

function TMPEGAUDIO.FGetXingBytes : Integer;
begin
    result:=0;
    if Mp3Type = mpVBR then result := XingHeader.bytes;
end;
        ...
        while not EOF (f) do begin
          ...
          case fver of
            $0100 : begin
                      ...
                      MPEGFile.FData.DurationExact := MPEGData1v0.Duration;
            ...
            $0101 : begin
                      ...
                      MPEGFile.FData.DurationExact := MPEGData1v1.Duration;
